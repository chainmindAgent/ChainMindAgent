...
def get_caption_text(protocols, date_str, interval):
    ...
    for i, p in enumerate(protocols[:10]):
        ...
        lines.append(f"{emoji} {i+1}. {p['name']} → {format_fees(p['fees'])}")
    
    list_text = "\n".join(lines)
    
    # Calculate total fees
    total = sum(p['fees'] for p in protocols[:10])
    
    return f"""📈 BNB Chain Fee Leaders | {interval_label}

Which protocols are generating the most fees on @BNBCHAIN? Here's the latest ranking:

{list_text}

💵 Total Top 10: {format_fees(total)}
📅 {date_str}
