...
def format_tvl(value):
    """Format TVL value to human readable string (e.g., $1.2M, $500K)"""
...
def fetch_tvl_data():
    ...
    print(f"[INFO] Fetching TVL data from DefiLlama protocols API...")
    ...
    for p in data:
        ...
        # BSC is often named 'Binance' or 'BSC' in DefiLlama
        bsc_tvl = chain_tvls.get("Binance") or chain_tvls.get("BSC")
        
        # Exclude CEX and other non-protocol entries if they appear
        if p.get("category") == "CEX":
            continue

        if bsc_tvl is not None and bsc_tvl > 1000: # Filter low TVL
            ...
            if group_key not in grouped_data:
                grouped_data[group_key] = {
                    ...
                    "tvl": 0,
                    "change": 0,
                    # Track max TVL component to keep best metadata
                    "max_tvl": -1 
                }
            ...
            # Update metadata if this component has higher TVL
    ...
    print("\n" + "=" * 80)
    print(f"🚀 TOP 10 TVL PROTOCOLS ON BSC")
    print("=" * 80)
    print(f"{'#':<3} | {'NAME':<25} | {'CATEGORY':<15} | {'TVL'}")
    print("-" * 80)
    for i, p in enumerate(top_protocols):
        print(f"{i+1:<3} | {p['name'][:23]:<25} | {p['category']:<15} | {format_tvl(p['tvl'])}")
    print("=" * 80 + "\n")
...
async def create_composite_image(protocols, output_path, date_str):
    ...
    if TEMPLATE_FILE.exists():
        ...
        bg_style = "background: linear-gradient(135deg, #1a2a6c 0%, #b21f1f 50%, #fdbb2d 100%);"
    ...
    <head>
        <style>
            ...
            * {{ margin: 0; padding: 0; box-sizing: border-box; }}
            ...
            .stat-tvl {{ color: #F0B90B; font-weight: 700; }}
    ...
    <body>
        ...
        <div id="container">
            <div id="label-badge">TOTAL VALUE LOCKED</div>
    """
    ...
    for i, protocol in enumerate(protocols):
        ...
        tvl_formatted = format_tvl(protocol["tvl"])
        
        stats_html = f'''
        <div class="protocol-stats">
            <span class="stat-cat">{protocol.get("category", "N/A")}</span>
            <span class="stat-tvl">{tvl_formatted}</span>
        </div>
        '''
    ...
    html_content += f"""
        ...
        <div id="footer">
            <span>{date_str}</span>
            <span>|</span>
            <span style="color: #F0B90B;">@ChainMindAgent</span>
        </div>
    </body>
    </html>
    """
    ...
    # Save debug HTML
    debug_html_path = Path(__file__).parent / "tvl_debug.html"
...
def get_caption_text(protocols, date_str):
    ...
    names = [f"{i+1}. {p['name']} ({format_tvl(p['tvl'])})" for i, p in enumerate(protocols[:10])]
    list_text = "\n".join(names)
    
    return f"""🏆 TOP PROTOCOLS BY TVL ON BNBCHAIN - {date_str}

Top @BNBCHAIN Ecosystem Projects by Total Value Locked (TVL) 👇

{list_text}

Source: DefiLlama & @ChainMindAgent
#BNBChain #DeFi #TVL"""
...
async def run_logic(timezone="UTC", json_output=False):
    ...
    output_path = f"tvl_rank_{int(datetime.now().timestamp())}.png"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate Top TVL Protocols on BNB Chain poster")
    ...
    parser.add_argument('--interval', default='24h', help='Interval (unused for TVL but kept for compatibility)')
