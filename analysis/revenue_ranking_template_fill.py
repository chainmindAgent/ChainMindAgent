...
def get_caption_text(protocols, date_str, interval):
    ...
    names = [f"{i+1}. {p['name']} ({format_revenue(p['revenue'])})" for i, p in enumerate(protocols[:10])]
    list_text = "\n".join(names)
    
    return f"""🏆 TOP REVENUE PROTOCOLS ON BNBCHAIN - {date_str}

Top @BNBCHAIN Ecosystem Projects by Revenue ({interval_label}) 👇

{list_text}

Source: DefiLlama & @ChainMindX
#BNBChain #DeFi #Revenue #ChainMind"""
