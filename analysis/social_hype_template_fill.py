...
def get_caption_text(tokens, timestamp_str):
    ...
    for i, t in enumerate(tokens[:10]):
        ...
        token_lines.append(f"{emoji} {cashtag}")
    
    token_list = "\n".join(token_lines)
    
    tweet_text = f"""🔥 BNBChain Token Dominance By Social Hype - {timestamp_str}

Top 10 Social Hype Tokens on @BNBCHAIN 👇
...
async def run_logic(post=False, force=False, timezone="UTC", interval="24", json_output=False, text_only=True):
    ...
    # Use full date for caption in text-only mode
    caption = get_caption_text(tokens, date_str)
    ...
    if post:
        ...
        print(f"[INFO] Dry run complete (Text Only).")
        print("\nCAPTION:")
        print(caption)
        return True
